...
import { DeleteVerseButton } from "@/components/dashboard/DeleteVerseButton";
import { togglePublishVerse } from "@/app/actions/quran";
...
const STATUS_STYLE: Record<string, string> = {
  ...
  Past: "bg-brand-100 text-brand-500",
};
...
export default async function QuranAdminPage() {
  ...
  // Verses are sorted by weekOf desc, so the first published one whose week
  // has begun is "Live" — everything published above it is "Scheduled" for
  // a future week, everything below it is "Past".
  const now = new Date();
  const liveId = verses.find((v) => v.publishedAt && v.weekOf <= now)?.id ?? null;
  function statusFor(v: (typeof verses)[number]) {
    if (!v.publishedAt) return "Draft";
    if (v.weekOf > now) return "Scheduled";
    if (v.id === liveId) return "Live";
    return "Past";
  }
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <span className={`rounded-full px-2.5 py-0.5 text-[11px] font-semibold ${STATUS_STYLE[status]}`}>
                            {status}
                          </span>
                        </div>
                        ...
                        <p dir="rtl" lang="ar" className="font-arabic mt-2 text-right text-lg leading-relaxed text-brand-950">
                          {v.arabicText}
                        </p>
                      ...
                      <div className="flex shrink-0 flex-wrap items-center gap-2">
                        ...
                        <DeleteVerseButton id={v.id} recitationCount={v._count.recitations} />
                      </div>
                    </div>
